...
# coding tips
+ 這題有大量指標運算，先弄清楚指標要怎麼指再開始寫程式碼
+ 變數非常多，所以好好命名變數也能幫助自己debug
...
struct treasure_node{
    ...
    ll v; // The remaining value of the treasure
};
...
int bin_search(ll x, int cur_layer){
    int l = 0, r = cur_layer;
    while (l != r){
        int m = (l + r) / 2;
        if (dep[m] >= x) r = m;
    ...
    while (q--){
        ...
        if (op == 1){
            ...
            if (head[cur] != NULL){
                ...
                dep[cur_layer+1] = dep[cur_layer] + head[cur]->d;
                cur = head[cur]->c;
            ...
            printf("%d\n", stack[bin_search(dep[cur_layer] - t, cur_layer)]);
        }
        else if (op == 4){
            ...
            int neg_id = bin_search(dep[cur_layer] - p, cur_layer);
            p -= dep[cur_layer];
